const Athena = (() => {
  ...
  function nav(active) {
    const links = [
      ['app.html','Dashboard'],['surveys.html','Surveys'],['survey-new.html','Create'],['survey-responses.html','Responses'],['survey-email.html','Email'],['subscription.html','Subscription'],['org-admin.html','Enterprise'],['app-admin.html','App Admin']
    ];
  ...
  function renderResults(results) {
    ...
    if (results.tags) {
      if (!results.tags.length) return '<p class="muted">No tag responses yet.</p>';
      const max = Math.max(...results.tags.map(t => t.count), 1);
      return `<div class="tag-cloud">${results.tags.map(t => `<span class="tag" style="font-size:${Math.max(14, Math.min(34, 14 + (t.count / max) * 20))}px">${escapeHtml(t.text)} <small>${t.count}</small></span>`).join('')}</div>`;
    }
