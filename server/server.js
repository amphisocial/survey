...
function reportAppUrl() {
  return String(process.env.APP_BASE_URL || APP_BASE_URL || 'https://survey.athenabot.ai').replace(/\/$/, '');
}
...
function addPdfFooter(doc) {
  // IMPORTANT:
  // Keep footer inside PDFKit's printable area.
  // If we draw too close to the physical bottom, PDFKit auto-adds a blank page.
  const oldX = doc.x;
  const oldY = doc.y;
  const url = reportAppUrl();
  const footerText = `Generated with Athena Survey • Create live AI polls at ${url}`;
...
function drawReportCoverPage(doc, survey, questions) {
  const url = reportAppUrl();

  doc.rect(0, 0, doc.page.width, 176).fill('#eef2ff');
  doc.circle(492, 72, 94).fill('#dbeafe');
  doc.circle(86, 126, 50).fill('#ede9fe');

  doc.roundedRect(50, 52, 48, 48, 14).fill('#2563eb');
  doc.font('Helvetica-Bold').fontSize(24).fillColor('#ffffff').text('A', 65, 64, {
    width: 20,
    align: 'center'
  });

  doc.font('Helvetica-Bold').fontSize(22).fillColor('#0f172a').text('Athena Survey', 112, 55);
  doc.font('Helvetica').fontSize(10).fillColor('#475569')
    .text('AI-powered live polls, QR surveys, and real-time audience insights.', 112, 84);

  doc.font('Helvetica-Bold').fontSize(28).fillColor('#0f172a')
    .text(pdfSafe(survey.title || 'Survey Results'), 50, 210, {
      width: 500,
      lineGap: 4
    });

  doc.font('Helvetica').fontSize(12).fillColor('#475569')
    .text(`Survey status: ${survey.status}`, 50, doc.y + 18)
    .text(`Questions: ${questions.length}`)
    .text(`Exported: ${new Date().toLocaleString()}`);

  if (survey.description) {
    doc.moveDown();
    doc.fontSize(11).fillColor('#334155').text(pdfSafe(survey.description), {
      width: 500,
      lineGap: 3
    });
  }

  const boxY = Math.max(doc.y + 34, 430);

  doc.roundedRect(50, boxY, 500, 138, 18).fill('#f8fafc').stroke('#e2e8f0');

  doc.font('Helvetica-Bold').fontSize(15).fillColor('#0f172a')
    .text('Create reports like this with Athena Survey', 74, boxY + 24);

  doc.font('Helvetica').fontSize(11).fillColor('#334155')
    .text('• Talk to AI and let it create the right survey questions', 74, boxY + 54)
    .text('• Share instantly with a QR code or public link')
    .text('• Show live charts, tag clouds, and audience results')
    .text('• Export branded PDF reports for teams, customers, and events');

  doc.font('Helvetica-Bold').fontSize(11).fillColor('#2563eb')
    .text(url, 74, boxY + 114, {
      link: url,
      underline: true
    });
...
function drawBarResultPdf(doc, optionText, count, pct, x, y, width) {
  const barWidth = width - 130;

  doc.font('Helvetica-Bold').fontSize(10).fillColor('#0f172a')
    .text(pdfSafe(optionText), x, y, { width: barWidth });

  doc.roundedRect(x, y + 18, barWidth, 10, 4).fill('#e2e8f0');
  doc.roundedRect(x, y + 18, Math.max(2, Math.round(barWidth * pct / 100)), 10, 4).fill('#2563eb');

  doc.font('Helvetica').fontSize(10).fillColor('#475569')
    .text(`${count} (${pct}%)`, x + barWidth + 14, y + 12, { width: 100 });

  return y + 42;
}

function drawTagCloudPdf(doc, tags, x, y, width) {
  if (!tags || !tags.length) {
    doc.font('Helvetica').fontSize(10).fillColor('#64748b').text('No tag responses yet.', x, y);
    return y + 24;
  }
  ...
  const colors = ['#4338ca', '#7c3aed', '#0369a1', '#047857', '#be123c', '#b45309', '#334155'];

  doc.roundedRect(x, y, width, 178, 18).fill('#f8fafc').stroke('#e2e8f0');
  ...
  for (let index = 0; index < sorted.length; index += 1) {
    ...
    const size = 13 + Math.round(weight * 18);
    const label = titleCaseTag(tag.text).slice(0, 32);

    doc.font('Helvetica-Bold').fontSize(size);
    const labelWidth = doc.widthOfString(label);
    const chipWidth = labelWidth + 22;
    const chipHeight = size + 12;

    if (cx + chipWidth > x + width - 20) {
      cx = x + 22;
      cy += rowHeight + 12;
      rowHeight = 0;
    }

    if (cy + chipHeight > bottomY) break;

    doc.roundedRect(cx - 6, cy - 5, chipWidth, chipHeight, 11).fill('#ffffff').stroke('#e2e8f0');

    doc.font('Helvetica-Bold').fontSize(size).fillColor(colors[index % colors.length])
      .text(label, cx + 4, cy, {
        width: chipWidth,
        lineBreak: false
      });
...
app.get('/api/surveys/:id/export.pdf', requireUser, requireSurveyOwner, async (req, res, next) => {
  try {
    ...
    const filename = `${String(req.survey.title || 'survey')
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, '-')
      .replace(/^-|-$/g, '')
      .slice(0, 60) || 'survey'}-results.pdf`;
    ...
    const doc = new PDFDocument({
      ...
      autoFirstPage: true,
      info: {
        Title: `${req.survey.title || 'Survey'} - Results`,
        Author: 'Athena Survey',
        Subject: `Survey results export from ${reportAppUrl()}`,
        Keywords: 'Athena Survey, AI polls, live surveys, QR surveys, audience insights'
      }
    });
    ...
    drawReportCoverPage(doc, req.survey, questions.rows);

    for (let i = 0; i < questions.rows.length; i += 1) {
      ...
      doc.font('Helvetica').fontSize(9).fillColor('#64748b')
        .text(`Question ${i + 1} • ${question.question_type} • ${question.status}`, 50, 58);

      doc.font('Helvetica-Bold').fontSize(18).fillColor('#0f172a')
        .text(pdfSafe(question.question_text), 50, 84, {
          width: 500,
          lineGap: 4
        });
      ...
      if (result?.tags) {
        doc.font('Helvetica-Bold').fontSize(12).fillColor('#334155').text('Tag cloud', 50, y);
        y += 24;
        y = drawTagCloudPdf(doc, result.tags, 50, y, 500);

        if (result.tags.length) {
          doc.font('Helvetica-Bold').fontSize(11).fillColor('#334155').text('Top themes', 50, y);
          y += 20;

          result.tags.slice(0, 10).forEach((tag) => {
            doc.font('Helvetica').fontSize(10).fillColor('#475569')
              .text(`${titleCaseTag(tag.text)} - ${tag.count}`, 64, y, { width: 430 });
            y = doc.y + 5;
          });
        }
      } else if (result?.options) {
        const total = Number(result.totalSelections || 0);

        doc.font('Helvetica-Bold').fontSize(12).fillColor('#334155')
          .text(`Total selections: ${total}`, 50, y);
        ...
        doc.font('Helvetica').fontSize(10).fillColor('#64748b').text('No results yet.', 50, y);
      }
    }
